...
//		bool isCheck = (IsDlgButtonChecked(GValue::G_hwnd_encoder, IDC_CHECKBOX_ENCODE) == BST_CHECKED);//SengMesage·½·¨ÓÉÓÚWM_COMMANDµÄÔ­Òò¿ÉÄÜ·¢ËÍÊ§°Ü 
		...
		char sText[2000000];// = {"ÌìÌìÏòÉÏ"}; //Êý×é×î´ó¾ÍÊÇ2000000ÁË£¬ÔÙ´óÔËÐÐÖ±½Ó±¨´í 
...
//		GetClientRect(GetDesktopWindow(),&rc);//ï¿½ï¿½ï¿½æ´°ï¿½Ú´ï¿½Ð¡
		...
		HBITMAP hBmp=CreateCompatibleBitmap(HDeskDC,width,height);//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Îªï¿½ï¿½ï¿½ï¿½ï¿½Ä´ï¿½ï¿½ï¿½Î»Í¼
		SelectObject(MemDC,hBmp);
		BitBlt(MemDC,0,0,width,height,HDeskDC,0,0,SRCCOPY);//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Í¼ï¿½ñ±£´æµ½ï¿½ÂµÄ»ï¿½Í¼ï¿½è±¸ï¿½ï¿½
		BITMAP BmpInfo;
