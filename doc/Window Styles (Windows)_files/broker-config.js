...
var _msdn = [ ["default.aspx",'1175',0.3],["library",'1176',0.3],["windowsazure",'1179',0.3],["magazine",'1180',0.3],["sharepoint",'1183',0.3],["sqlserver",'1184',0.3],["subscriptions",'1185',0.3],["vstudio",'1189',0.3] ];
//(TECHNET)-/en-us/(TIER 5) - CUSTOM - Make frequency changes here for tehnet.microsoft.com/en-us/…(various paths)
//Array [PATH,SITE_CODE,FREQ]
var _technet = [ ["default.aspx",'1219',0.3],["library",'1192',0.3],["cloud",'2201',0.3],["network",'2200',0.3],["sbs",'2199',0.3],["windowsserver",'1194',0.0465],["forefront",'1011',0.3], ["sharepoint",'1196',0.3],["sqlserver",'1197',0.3],["systemcenter",'1198',0.3],["windows",'1199',0.3],["scriptcenter",'1200',0.3],["security",'1202',0.3],["subscriptions",'1206',0.3],["sysinternals",'1203',0.3],["ie",'1220',0.3],["exchange",'1221',0.3],["lync",'1566',0.3]  ];
...
function checkWTSP(){
	 ...
	 if(_Site == '1176'){
		...
		if(!(/technet_(library_(windowsserver_|sqlserver_|sharepoint|forefront|lync|win7_|security_|ie_|exchangeserver_|windowsserveressentials_))|technet_sharepoint_library|sto_technet_systemcenter_|sto_technet_windowsclient_library_|scriptcenter_technet/i.test(_wtsp))){	
				_halt=true;
...
if(/social\.msdn\.microsoft\.com\/forums\/en-us/i.test(SR_url_stripped[0])){
	if(/windowsazure/i.test(srchMSForumIroot)){_pipeIn="Windows Azure Developer Center"; _Hweight=9;}
	else if(/office/i.test(srchMSForumIroot)){_pipeIn="Office Developer Center"; _Hweight=13;}
	else if(/sharepoint/i.test(srchMSForumIroot)){_pipeIn="SharePoint Developer Center"; _Hweight=17;}
	else if(/sqlserver/i.test(srchMSForumIroot)){_pipeIn="SQL Server Developer Center"; _Hweight=20;}
	else if(/vstudio/i.test(srchMSForumIroot)){_pipeIn="Visual Studio Developer Center"; _Hweight=29;}
}
...
else if(_Site=='1180'){_pipeIn="MSDN Magazine";_Hweight=10;}
...
else if(_Site=='1185'){_pipeIn="MSDN Subscription";_Hweight=21;}
...
if(/social\.technet\.microsoft\.com\/forums\/en-us/i.test(SR_url_stripped[0])){
	if(/windowsserveressentials/i.test(srchMSForumIroot)){_pipeIn="Windows Server Essentials TechCenter";_Hweight=11;}
	else if(/windowsserver/i.test(srchMSForumIroot)){_pipeIn="Windows Server TechCenter";_Hweight=8;}
	else if(/office/i.test(srchMSForumIroot)){_pipeIn="Office TechCenter";_Hweight=14;}
	else if(/exchange/i.test(srchMSForumIroot)){_pipeIn="Exchange Server TechCenter";_Hweight=22;}
	else if(/systemcenter/i.test(srchMSForumIroot)){_pipeIn="System Center TechCenter";_Hweight=25;}
	else if(/sqlserver/i.test(srchMSForumIroot)){_pipeIn="SQL Server TechCenter";_Hweight=28;}
	else if(/ie/i.test(srchMSForumIroot)){_pipeIn="Internet Explorer TechCenter";_Hweight=31;}
	else if(/sharepoint/i.test(srchMSForumIroot)){_pipeIn="SharePoint TechCenter";_Hweight=34;}
	else if(/forefront/i.test(srchMSForumIroot)){_pipeIn="Forefront TechCenter";_Hweight=41;}
	else if(/lync/i.test(srchMSForumIroot)){_pipeIn="Lync TechCenter";_Hweight=46;}
}
...
COMSCORE.SiteRecruit.Broker.config={version:"5.0.3",testMode:false,cookie:{name:"msresearch",path:"/",domain:".microsoft.com",duration:90,rapidDuration:0,expireDate:""},thirdPartyOptOutCookieEnabled:false,prefixUrl:"",Events:{beforeRecruit:function(){}},mapping:[{m:"(code\\.msdn\\.microsoft\\.com|cpapp02)/(WINDOWSAPPS|WINDOWSDESKTOP|WINDOWSHARDWARE|IE)",c:"inv_c_BLANK.js",f:0,p:2,halt:true},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/en-us/((default\\.aspx)|(library|windowsazure|magazine|sqlserver|subscriptions|sharepoint|vstudio))",c:"inv_c_MSDN-p218260443_TIER5.js",f:_Freq,p:1,halt:_halt},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/en-us/(ie|windows)|(library/((ie)|(windows/(hardware|desktop|apps))))",c:"inv_c_BLANK.js",f:0,p:3,halt:true},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/en-us/office",c:"inv_c_MSDN-p218260443-1181.js",f:0.3,p:2},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/en-us/office/apps",c:"inv_c_MSDN-p218260443-2009.js",f:0.3,p:3},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/en-us/office365",c:"inv_c_MSDN-p218260443-2185.js",f:0.3,p:3},{m:"/(sr-msdn|msdnstage|msdntest|msdnlive|msdn\\.microsoft)[\\w\\.-]+/ja-jp",c:"inv_c_MSDN-p15466742-JA.js",f:0.0056,p:0},{m:"(.*?social\\.msdn\\.microsoft)[\\w\\.-/]+/Forums/en-(us|US)",c:"inv_c_SC-MSDN-p218260443-Tier1.js",f:0.3,p:1,prereqs:{content:[{"element":"meta","attrib":"content","attribValue":"office|vstudio|sqlserver|sharepoint|windowsazure"}],cookie:[],externalDomain:[]}},{m:"(.*?social\\.msdn\\.microsoft)[\\w\\.-/]+/forums/en-(us|US)/category/(windowsdesktopdev|windowshardwaredev|iedevelopment|windowsapps)",c:"inv_c_BLANK.js",f:0,p:3,halt:true},{m:"(.*?social\\.technet\\.microsoft)[\\w\\.-/]+/(Forums/(en/ITCG/threads|scriptcenter/en-US/home))",c:"inv_c_SC-TN-p218292485-1201.js",f:0.3,p:1,prereqs:{content:[{"element":"meta","attrib":"content","attribValue":"scriptcenter"}],cookie:[],externalDomain:[]}},{m:"(.*?social\\.technet\\.microsoft|sr-technet)[\\w\\.-]+/forums/en-(us|US)",c:"inv_c_SC-TN-p218292485-TIER1.js",f:0.3,p:1,prereqs:{content:[{"element":"meta","attrib":"content","attribValue":"forefront|lync|systemcenter|ie|sharepoint|sqlserver|exchange|office|windowsserveressentials"}],cookie:[],externalDomain:[]}},{m:"(.*?social\\.technet\\.microsoft|sr-technet)[\\w\\.-]+/forums/en-(us|US)",c:"inv_c_SC-TN-p218292485-TIER2.js",f:0.0465,p:1,prereqs:{content:[{"element":"meta","attrib":"content","attribValue":"windowsserver"}],cookie:[],externalDomain:[]}},{m:"(.*?social\\.technet\\.microsoft|sr-technet)[\\w\\.-]+/forums/windows/en-US",c:"inv_c_SC-TN-p218292485-1224.js",f:0.3,p:2,prereqs:{content:[{"element":"meta","attrib":"content","attribValue":"windows"}],cookie:[],externalDomain:[]}},{m:"/(sr-technet|tnstage|tnlive|tntest|technet\\.microsoft)[\\w\\.-]+/en-us/office/ocs",c:"inv_c_BLANK.js",f:0,p:3,halt:true},{m:"/(sr-technet|tnstage|tnlive|tntest|technet\\.microsoft)[\\w\\.-]+/en-us/((default\\.aspx)|(windows($|/))|(library|cloud|network|sbs|windowsserver|forefront|sharepoint|sqlserver|systemcenter|scriptcenter|security|subscriptions|sysinternals|ie|exchange|lync))",c:"inv_c_TechNet-p218292485.js",f:_Freq,p:1,halt:_halt},{m:"/(sr-technet|tnstage|tnlive|tntest|technet\\.microsoft)[\\w\\.-]+/en-us/office",c:"inv_c_TN-p218292485-1195.js",f:0.3,p:2},{m:"/(sr-technet|tnstage|tnlive|tntest|technet\\.microsoft)[\\w\\.-]+/en-us/office365",c:"inv_c_TN-p218292485-1569.js",f:0.3,p:3},{m:"/(sr-technet|tnstage|tnlive|tntest|technet\\.microsoft)[\\w\\.-]+/ja-jp",c:"inv_c_TECHNET-p15466742-p81712691-JA.js",f:0.0056,p:1},{m:"//[\\w\\.-]+/en-us/server-cloud/windows-server",c:"inv_c_WWW-p218292485.js",f:0.3,p:0}]};
